...
export class AddressUserService {
  ...
  async createAddress(
    createAddressDto: CreateAddressUserDto,
    ...
    try {
      if (
        ...
        throw new UnauthorizedException('Bạn không có quyền thêm mới địa chỉ');
      if (createAddressDto.is_default) {
        const listAddress = await this.addressUserRepository.find({
          where: { user_id: createAddressDto.user_id },
        });
  ...
  async getAddressDefault(id: number): Promise<ApiResponse<AddressUserEntity>> {
    try {
      let address = await this.addressUserRepository.findOne({
  ...
  async getUserAddress(id: number): Promise<ApiResponse<AddressUserEntity>> {
    try {
      let address = await this.addressUserRepository.findOne({
        where: { id: id },
      });
  ...
  async updateAddress(
    updateAddressDto: UpdateAddressUserDto,
    currentUser: UserEntity,
  ): Promise<ApiResponse<AddressUserEntity>> {
    try {
      ...
      if (
        ...
        throw new UnauthorizedException('Bạn không có quyền chỉnh sửa địa chỉ');
  ...
  async updateAddressDefault(
    ...
    try {
      const address = await this.addressUserRepository.findOne({
        where: { id: updateAddressDefaultDto.id },
      });
      if (!address) throw new NotFoundException('Địa chỉ không tồn tại');
      if (
        !this.validateUserService.checkRoleAndCurrentUser(
  ...
  async deleteAddress(
    ...
    try {
      ...
      if (
        ...
        throw new UnauthorizedException('Bạn không có quyền thêm mới địa chỉ');
      if (address.is_default) {
        const address_to_default = await this.addressUserRepository.findOne({
          where: { id: deleteAddressDto.id_address_default },
        });
        if (!address_to_default)
