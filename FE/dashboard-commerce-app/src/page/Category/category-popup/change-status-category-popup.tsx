...
import { changeStatusCategory } from "../../../services/category/category-service";
...
const CreateCategoryPopup: React.FC<ChangeStatusCategoryPopup> = ({ open, onClose, category, currentPage }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.status == HttpStatusCode.Ok) {
        ...
        category.status == 1 ? Toast.ToastSuccess("Tạm ngưng danh mục thành công") : Toast.ToastSuccess("Bật hoạt động danh mục thành công");
        onClose();
      } else {
  ...
  return (
    <Popup title=" Xác nhận" open={open} onClose={onClose} onSubmit={handleSubmit} submitText="Xác nhận" cancelText="Huỷ" maxWidth="md">
